...
async function profileRoutes(fastify) {
    ...
    fastify.get('/api/users/:username/profile', authOpts, async (request, reply) => {
        ...
        try {
            ...
            const totalMatchesResult = await dbGet(`SELECT COUNT(*) as count FROM matches WHERE (player1_id IN (${participantIdPlaceholders}) OR player2_id IN (${participantIdPlaceholders})) AND status = 'completed'`, [...participantIds, ...participantIds]);
