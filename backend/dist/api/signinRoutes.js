...
async function signinRoutes(app) {
    app.post('/api/signin', {
        config: {
            rateLimit: {
        ...
        if (!(0, validators_1.validateUsername)(username)) { // Also validate username format on sign-in
            return reply.status(400).send({ message: 'Invalid username format.' });
        }
        try {
            const user = await new Promise((resolve, reject) => {
                database_1.default.get('SELECT id, username, password_hash, twofa_enabled, twofa_secret FROM users WHERE username = ?', [username.toLowerCase()], (err, row) => {
                    if (err)
                        return reject(err);
                    resolve(row);
                });
            ...
            const passwordMatch = await (0, pass_service_1.verifyPassword)(password, user.password_hash);
            if (!passwordMatch) {
                return reply.status(401).send({ message: 'Invalid username or password.' });
            }
            // --- MFA Logic ---
            if (user.twofa_enabled) {
                if (!mfaCode) {
                    ...
                    const pendingToken = app.jwt.sign({ id: user.id, username: user.username, tfa: 'pending' }, { expiresIn: '5m' } // This token is short-lived
                    );
                ...
                try {
                    const decodedToken = request.server.jwt.verify(request.cookies.authToken || '');
                ...
                if (!user.twofa_secret || !(0, _2fact_1.validateMfaToken)(user.twofa_secret, mfaCode)) {
