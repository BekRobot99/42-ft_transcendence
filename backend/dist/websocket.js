...
const GameStateValidator_1 = require("./services/GameStateValidator");
const GameErrorHandler_1 = require("./services/GameErrorHandler");
const PerformanceMonitor_1 = require("./services/PerformanceMonitor");
const PerformanceDashboard_1 = require("./services/PerformanceDashboard");
const PerformanceAnalytics_1 = require("./services/PerformanceAnalytics");
...
const gameStateValidator = new GameStateValidator_1.GameStateValidator(); // Global state validator
const gameErrorHandler = new GameErrorHandler_1.GameErrorHandler(); // Global error handler
const performanceMonitor = new PerformanceMonitor_1.PerformanceMonitor(); // Global performance monitor
const performanceDashboard = new PerformanceDashboard_1.PerformanceDashboard(performanceMonitor); // Performance dashboard
const performanceAnalytics = new PerformanceAnalytics_1.PerformanceAnalytics(performanceMonitor); // Performance analytics
...
async function safeExecute(operation, errorType, context) {
    try {
        ...
        await gameErrorHandler.logError(errorType, `${context.operation} failed: ${errorMessage}`, {
            originalError: error,
            context
...
async function safeSend(connection, message, gameId, playerId) {
    try {
        if (!connection || connection.readyState !== connection.OPEN) {
            await gameErrorHandler.logError('websocket_connection_lost', 'Attempted to send message on closed connection', { messageType: message.type || 'unknown' }, gameId, playerId);
            return false;
        }
        ...
        await gameErrorHandler.logError('websocket_send_failed', 'Failed to send WebSocket message', {
...
async function broadcastToGame(gameId, message, excludePlayerId) {
    const room = gameRooms.get(gameId);
    if (!room) {
        await gameErrorHandler.logError('unknown_error', 'Attempted to broadcast to non-existent game room', { gameId }, gameId);
        return;
    }
    const promises = room.players.map(async (player) => {
        ...
        if (sockets) {
            const socketPromises = Array.from(sockets).map(socket => safeSend(socket, message, gameId, playerId.toString()));
...
async function createAIGameRoom(humanPlayerId, difficulty) {
...
async function handleGameMessage(connection, userId, message) {
        ...
        switch (message.event) {
            case 'paddle_move':
                ...
                if (player && !player.isAI && synchronizer) {
                    ...
                    const validationResult = gameStateValidator.validateGameState(gameId, room.gameState, room.players);
            ...
            case 'validate_state':
                ...
                if (room && message.data) {
                    ...
                    if (frontendState) {
                        ...
                        const comparisonResult = gameStateValidator.compareStates(gameId, frontendState, room.gameState);
    ...
    performanceDashboard.updateConnectionCount(exports.activeUsers.size);
}
...
async function realtimeRoutes(fastify) {
        ...
        try {
            ...
            return {
                ...
                summary: {
                    ...
                    estimatedTotalGain: opportunities.reduce((sum, o) => sum + o.potentialGain, 0)
                },
                responseTime: Math.round(responseTime * 100) / 100,
                timestamp: Date.now()
            };
    ...
    fastify.get('/api/performance/report', async (request, reply) => {
        const startTime = performance.now();
        try {
            const timeWindow = request.query?.timeWindow ?
                parseInt(request.query.timeWindow) : 3600000; // Default 1 hour
    ...
    fastify.get('/api/ws/performance', { websocket: true }, async (connection, request) => {
            ...
            try {
                ...
                connection.socket.send(JSON.stringify({
                    ...
                    data: {
                        summary,
                        healthOverview,
                        connectionCount: exports.activeUsers.size,
        ...
        const onBottleneck = (bottlenecks) => {
            connection.socket.send(JSON.stringify({
        ...
        const onRecommendation = (recommendation) => {
            connection.socket.send(JSON.stringify({
    ...
    fastify.get('/api/ws/game', { websocket: true }, async (connection, request) => {
        ...
        connection.on('message', async (messageBuffer) => {
            ...
            try {
                ...
                await gameErrorHandler.logError('websocket_message_invalid', 'Failed to process WebSocket message', {
                    error: error instanceof Error ? error.message : 'Unknown error',
                    rawMessage: messageBuffer.toString()
                }, undefined, userId?.toString());
    ...
    fastify.get('/api/game-health', async (request, reply) => {
        try {
            ...
            gameErrorHandler.updateHealthStats(exports.activeUsers.size, gameRooms.size, healthMetrics.averageLatency);
            ...
            return {
                ...
                systemHealth: {
                    ...
                    games: {
                        ...
                        activeUsers: exports.activeUsers.size,
            ...
            await gameErrorHandler.logError('unknown_error', 'Failed to generate health report', { error: error instanceof Error ? error.message : 'Unknown error' });
            reply.status(500);
