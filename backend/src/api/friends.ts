...
export default async function friendsRoutes(app: FastifyInstance) {
    ...
    app.post('/api/friends/request', authOpts, async (request: any, reply: FastifyReply) => {
        const requesterId = request.user.id;
        const { username: addresseeUsername } = request.body as { username: string };
        ...
        try {
            // Find addressee user
            const addressee = await new Promise<any>((resolve, reject) => {
                databaseConnection.get('SELECT id FROM users WHERE username = ?', [addresseeUsername.toLowerCase()], (err, row) => {
            ...
            const existingRequest = await new Promise<any>((resolve, reject) => {
    ...
    app.get('/api/friends', authOpts, async (request: any, reply: FastifyReply) => {
            ...
            const friends = await new Promise<any[]>((resolve, reject) => {
                const query = `
                    ...
                    WHERE fr.status = 'accepted'
                      ...
                      AND u.id != ?
                `;
                databaseConnection.all(query, [userId, userId, userId], (err, rows) => {
                    if (err) return reject(err);
                    resolve(rows);
                });
            });
    ...
    app.get('/api/friends/requests', authOpts, async (request: any, reply: FastifyReply) => {
        const userId = request.user.id;
        try {
            // Incoming requests
            const incoming = await new Promise<any[]>((resolve, reject) => {
                const query = `
                ...
                `;
                databaseConnection.all(query, [userId], (err, rows) => {
                    if (err) return reject(err);
                    resolve(rows);
                });
            });
            ...
            // Outgoing requests
            const outgoing = await new Promise<any[]>((resolve, reject) => {
                const query = `
                    SELECT fr.id, u.username, u.display_name, u.avatar_path
                    FROM friend_requests fr
    ...
    app.put('/api/friends/request/:requestId', authOpts, async (request: any, reply: FastifyReply) => {
        const userId = request.user.id;
    ...
    // Remove a friend
    app.delete('/api/friends/:friendId', authOpts, async (request: any, reply: FastifyReply) => {
        const userId = request.user.id;
