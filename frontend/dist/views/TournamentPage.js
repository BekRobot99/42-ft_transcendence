...
function fetchTournament(id) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`/api/tournaments/${id}`);
            if (!res.ok) {
                const err = yield res.json();
                throw new Error(err.message || 'Failed to fetch tournament data');
            }
            tournamentState = yield res.json();
            render();
        }
        catch (error) {
            console.error(error);
...
function renderCreationForm() {
    ...
    `;
    appContainer === null || appContainer === void 0 ? void 0 : appContainer.appendChild(formContainer);
    ...
    form.addEventListener('submit', (e) => __awaiter(this, void 0, void 0, function* () {
        ...
        const playerCount = parseInt(formData.get('playerCount'), 10);
        const gameMode = formData.get('gameMode');
        try {
            const res = yield fetch('/api/tournaments', {
                method: 'POST',
...
function renderLobby() {
    ...
    let participantListHTML = participants.map((p) => `<li class="p-2 bg-gray-200 rounded">${p.alias} ${p.user_id === tournamentState.creator_id ? `(${translate('Host', 'Gastgeber', 'Hôte')})` : ''}</li>`).join('');
    ...
    `;
    appContainer === null || appContainer === void 0 ? void 0 : appContainer.appendChild(lobbyContainer);
    ...
    if (!canStart) {
        ...
        form.addEventListener('submit', (e) => __awaiter(this, void 0, void 0, function* () {
            e.preventDefault();
            const alias = form.elements.namedItem('alias').value;
            const errorEl = document.getElementById('lobby-error');
            errorEl.classList.add('hidden');
            try {
                const res = yield fetch(`/api/tournaments/${tournamentState.id}/participants`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                yield fetchTournament(tournamentState.id);
            }
            catch (error) {
                errorEl.textContent = error.message;
                errorEl.classList.remove('hidden');
        ...
        startButton.addEventListener('click', () => __awaiter(this, void 0, void 0, function* () {
            ...
            errorEl.classList.add('hidden');
            try {
                const res = yield fetch(`/api/tournaments/${tournamentState.id}/start`, { method: 'POST' });
                if (!res.ok) {
                    const err = yield res.json();
...
function renderBracket() {
    ...
    tournamentState.matches.forEach((match) => {
        ...
        if (!rounds[match.round])
            rounds[match.round] = [];
        ((_a = rounds[match.round]) !== null && _a !== void 0 ? _a : []).push(match);
    });
    ...
    if (tournamentState.status === 'completed') {
        const winner = (_a = tournamentState.matches.find((m) => m.round === Object.keys(rounds).length)) === null || _a === void 0 ? void 0 : _a.winner_alias;
    ...
    appContainer === null || appContainer === void 0 ? void 0 : appContainer.appendChild(bracketContainer);
...
function playMatch(match) {
    ...
    const gameOptions = {
        ...
        onGameEnd: (result) => __awaiter(this, void 0, void 0, function* () {
            const winnerId = result.winnerName === match.player1_alias ? match.player1_id : match.player2_id;
            try {
                yield fetch(`/api/matches/${match.id}/winner`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        winnerId,
...
export function renderTournamentView(container) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (tournamentId) {
            yield fetchTournament(parseInt(tournamentId, 10));
        }
        else {
            tournamentState = null;
            render();
...
//# sourceMappingURL=TournamentPage.js.map