...
function createProfileCard(user, ...buttons) {
...
export function renderSocialView(container) {
    ...
    `;
        ...
        addFriendForm.addEventListener('submit', (e) => __awaiter(this, void 0, void 0, function* () {
            e.preventDefault();
            ...
            addFriendFeedback.className = 'text-sm mt-2';
            try {
                const res = yield fetch('/api/friends/request', {
                    method: 'POST',
        ...
        try {
            const [requestsRes, friendsRes] = yield Promise.all([
            ...
            const { incoming, outgoing } = yield requestsRes.json();
            const friends = yield friendsRes.json();
...
function renderFriendRequests(container, incoming, outgoing, rerenderCallback) {
    container.innerHTML = '';
    ...
    if (incoming.length > 0) {
        ...
        incoming.forEach(req => {
            ...
            `;
            acceptButton.onclick = () => __awaiter(this, void 0, void 0, function* () {
                yield fetch(`/api/friends/request/${req.id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
            ...
            `;
            declineButton.onclick = () => __awaiter(this, void 0, void 0, function* () {
                yield fetch(`/api/friends/request/${req.id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
    ...
    if (outgoing.length > 0) {
        ...
        outgoing.forEach(req => {
            ...
            `;
            cancelButton.onclick = () => __awaiter(this, void 0, void 0, function* () {
                yield fetch(`/api/friends/request/${req.id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
...
// Render Friends List
function renderFriendList(container, friends, rerenderCallback) {
    container.innerHTML = '';
    ...
    if (friends.length === 0) {
        ...
        friends.forEach(friend => {
            ...
            `;
            removeButton.onclick = () => __awaiter(this, void 0, void 0, function* () {
                if (confirm(translate(`Are you sure you want to remove ${friend.username}?`, `Bist du sicher, dass du ${friend.username} entfernen möchtest?`, `Êtes-vous sûr de vouloir supprimer ${friend.username} ?`))) {
                    yield fetch(`/api/friends/${friend.id}`, {
                        method: 'DELETE',
                        credentials: 'include',
                    });
                    rerenderCallback();
