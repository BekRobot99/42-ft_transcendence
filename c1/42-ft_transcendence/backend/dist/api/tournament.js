...
async function tournamentRoutes(fastify) {
    ...
    fastify.post('/api/tournaments', authOpts, async (request, reply) => {
        const { name, playerCount, gameMode } = request.body;
        ...
        if (!(0, validators_1.ValidTournamentName)(name)) {
        ...
        try {
            ...
            const result = await dbRun('INSERT INTO tournaments (name, number_of_players, creator_id, game_type) VALUES (?, ?, ?, ?)', [name, playerCount, creatorId, gameMode]);
            const tournamentId = result.lastID;
            // Add creator as the first participant
            await dbRun('INSERT INTO tournament_participants (tournament_id, user_id, alias) VALUES (?, ?, ?)', [tournamentId, creatorId, creator.display_name]);
    ...
    fastify.post('/api/tournaments/:id/participants', authOpts, async (request, reply) => {
        ...
        if (!(0, validators_1.ValidAlias)(alias)) {
    ...
    // Start a tournament
    fastify.post('/api/tournaments/:id/start', authOpts, async (request, reply) => {
        const tournamentId = parseInt(request.params.id, 10);
        const userId = request.user.id;
        try {
            const tournament = await dbGet('SELECT * FROM tournaments WHERE id = ?', [tournamentId]);
            if (!tournament)
                return reply.status(404).send({ message: 'Tournament not found.' });
            if (tournament.creator_id !== userId)
                return reply.status(403).send({ message: 'Only the creator can start the tournament.' });
            if (tournament.status !== 'pending')
                return reply.status(400).send({ message: 'Tournament has already started or is completed.' });
            const participants = await dbAll('SELECT * FROM tournament_participants WHERE tournament_id = ?', [tournamentId]);
            if (participants.length !== tournament.number_of_players) {
            ...
            for (let i = 0; i < participants.length; i += 2) {
                ...
                await dbRun('INSERT INTO matches (tournament_id, round, match_in_round, player1_id, player2_id) VALUES (?, ?, ?, ?, ?)', [tournamentId, 1, i / 2 + 1, player1.id, player2.id]);
            }
    ...
    fastify.get('/api/tournaments/:id', authOpts, async (request, reply) => {
        const tournamentId = parseInt(request.params.id, 10);
        try {
            const tournament = await dbGet('SELECT * FROM tournaments WHERE id = ?', [tournamentId]);
            if (!tournament)
    ...
    // Report match winner
    fastify.post('/api/matches/:matchId/winner', authOpts, async (request, reply) => {
        ...
        try {
            ...
            if (!tournament)
                return reply.status(404).send({ message: 'Tournament not found.' });
            if (tournament.creator_id !== userId)
                return reply.status(403).send({ message: 'Only the creator can report results.' });
            if (match.status !== 'pending')
                return reply.status(400).send({ message: 'Match result already reported.' });
            if (winnerId !== match.player1_id && winnerId !== match.player2_id)
                return reply.status(400).send({ message: 'Winner is not a participant in this match.' });
            ...
            if (completedRoundMatches.length === totalRoundMatches.length) {
                ...
                if (winners.length > 1) {
                    for (let i = 0; i < winners.length; i += 2) {
                        await dbRun('INSERT INTO matches (tournament_id, round, match_in_round, player1_id, player2_id) VALUES (?, ?, ?, ?, ?)', [match.tournament_id, match.round + 1, i / 2 + 1, winners[i], winners[i + 1]]);
                    }
                }
                else {
